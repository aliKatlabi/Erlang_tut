...
% as you remember the last expression is the return value 
...
%			trying to bind the other 4 to X but it's already bound, so it compares it (as we know we can bind again with equal value)
%			so X can take the other 4 => first block executed returning true.
%			otherwise if it's other value than 4 it will go to the other block
...
% it is possiple to use = in the function head 
...
true -> always_does  %% this is Erlang's if's 'else!' or 'default case'
end.
...
% case..of is like the whole function head 
