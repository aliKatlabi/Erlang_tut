...
-compile([debug_info,export_all]). % same as compiling from shell with these flages
				   % like c(module_name,[debug_info,export_all]).
...
%% Body is one more erlang expressions(sxpr followed by . and space) seperated by commas or semi-colons
...
%% OR we can import io like .. ( import(io , [format/1])
